...
public class MovementNoise : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        var magnitudeX = Mathf.Lerp(_magnitudes.x * 0.25f, _magnitudes.x, 1f - (_roadGenerator.roadSpeed / _speedSelector.PlayerCollisionMaxCalibratedSpeed));
        var magnitudeY = _magnitudes.y;
        var magnitude = new Vector2(magnitudeX, magnitudeY);

        var rateX = Mathf.Lerp(_rates.x * 0.25f, _rates.x, (_roadGenerator.roadSpeed / _speedSelector.PlayerCollisionMaxCalibratedSpeed));
        var rateY = _rates.y;
